...
print("  -> Some random number to check if seed fix works: %f (numpy) ; %f (tf)" % (
np.random.random(), tf.random.uniform((1, 1))[0][0]))
...
def wann():
    ...
    mylist.insert(END, wann_genome.score(X_test, Y_test))
...
if __name__ == "__main__":
    root = Tk()
    root.title('Neuro Evolution WANN Search')
    ...
    button_1 = Button(root, text='Start WANN Search', width='30', height='5', command=wann)
