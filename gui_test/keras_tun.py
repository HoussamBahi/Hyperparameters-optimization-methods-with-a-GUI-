...
def architecture(hyperparam):
    ...
    lrng = hyperparam.Choice('lr', values=optimi)
    actt= hyperparam.Choice('activation', values=activi)
    inii= hyperparam.Choice('kernel_initializer', values=ini)

    for i in range(layer):
        model.add(
            tf.keras.layers.Dense(neuron, input_dim=11, kernel_initializer=inii, activation=actt))
...
def ker():
 ...
 tuner_search = RandomSearch(architecture,
                          ...
                          executions_per_trial=Batch(),
                          max_trials=iteration())

 tuner_search.search(X_train,Y_train,epochs=epoch(),validation_split=splits())
 print("bbest hyperparams are")
...
if __name__ == "__main__":
    ...
    button_1 = Button(root, text='Start Keras Tuner Search', width='20', height='5', command=ker)
    button_1.grid(row=13, column=4)
    lab1= Label(root, text="Layers").grid(row=0)
    lab2= Label(root, text="learning rate").grid(row=1)
    lab3= Label(root, text="Epoch").grid(row=2)
    lab4= Label(root, text="Batch size").grid(row=3)
    lab5 = Label(root, text="kernel_initializer").grid(row=4)
    lab6 = Label(root, text="Neurons").grid(row=5)
    lab7 = Label(root, text="Num of pre samples").grid(row=6)
    lab8 = Label(root, text="activation").grid(row=7)
    ...
    button_2 = Button(root, text='add layers  ', width='20', height='2', command=layers)
    button_2.grid(row=0, column=2)

    button_3 = Button(root, text='Learning rate ', width='20', height='2', command=opti)
    ...
    button_6 = Button(root, text='kernel_initializer ', width='20', height='2', command=init_mod)
    ...
    button_9 = Button(root, text='activation ', width='20', height='2', command=acti)
