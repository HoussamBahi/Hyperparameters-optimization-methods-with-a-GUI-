...
def architecture(init_mode='uniform',nb_laters=2,optimizer='SGD',nb_neurons=50,activation='softsign'):
...
def grid_train():
 ...
 param_grid = dict(init_mode=ini,nb_laters=layer,optimizer=optimi,nb_neurons=neuron,activation=activi)
 grid = GridSearchCV(estimator=model, param_grid=param_grid,n_jobs=1,
                    cv=splits(), verbose=10)
 #grid_results = grid.fit(X_train, Y_train)
 #scroll=Scrollbar(root,text=grid.fit(X_train, Y_train)).grid(row=5)
 #txt = scrolledtext(root,width=40,height=10)
 #
 # txt.insert(grid.fit(X_train, Y_train))
 #labbest = Label(root, text=grid.fit(X_train,Y_train)).grid(row=5,column=2)


 grid_results = grid.fit(X_train, Y_train)
 print("Best: {0}, using {1}".format(grid_results.best_score_, grid_results.best_params_))
 lab0 = Label(root, text="Best: {0}, using {1}".format(grid_results.best_score_, grid_results.best_params_)).grid(row=14,column= 2)
 ...
 mylist = Listbox(root,width=160, yscrollcommand=scrollbar.set)
 ...
 mylist.grid(row=8,column=2, rowspan=4,
                       columnspan=6)
...
if __name__ == "__main__":
    ...
    mylist.grid(row=8, column=2, rowspan=4,
                columnspan=6)
    scrollbar.config(command=mylist.yview)
    button_1 = Button(root, text='Start Grid Search', width='20', height='5', command=grid_train)
    button_1.grid(row=13, column=6)
...
preds = architecture.predict(X_test)[:, 0]
